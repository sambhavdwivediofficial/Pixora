...
pub fn run_tauri_app() {
    ...
    // Start embedded HTTP server on port 8599 in background thread
    std::thread::spawn(|| {
        ...
        rt.block_on(async {
            ...
            tracing::info!("Pixora embedded server → http://{}", addr);
    ...
    // Small delay — let server bind before window opens
